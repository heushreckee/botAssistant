...
class BotDB:
    ...
    '''def get_records_day(self, user_id, k):
        result = self.cursor.execute(
            "SELECT * FROM `records` WHERE `users_id` = ? AND `date` BETWEEN datetime(DATE_SUB(NOW(), INTERVAL 2 day), 'start of day') AND "
            "datetime(DATE_SUB(NOW(), INTERVAL 1 day), 'start of day') ORDER BY `date`",
            (self.get_user_id(user_id),))
        return result.fetchall()'''

    def get_records_day(self, user_id, k):
        result = self.cursor.execute(
            f" SELECT * FROM `records` WHERE `users_id` = ? AND `date` BETWEEN datetime('now', '{-k} days', 'start of day') AND "
            f" datetime('now', '{-k + 1} days', 'start of day') ORDER BY `date` ",
            (self.get_user_id(user_id),))
        return result.fetchall()
